...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const getBreadcrumbs = () => {
    ...
    if (pathname === '/admin/settings') return [{ label: 'Global Calculation Engine', href: '/admin/settings' }];
    if (pathname === '/admin/help') return [{ label: 'Operations & SOP Guide', href: '/admin/help' }];
    return [{ label: 'Demand Intelligence', href: '/admin' }];
  };

  const navItems = [
    { label: 'Demand Intelligence', href: '/admin', icon: Sparkles, badge: 'Live AI' },
    { label: 'Auction Inventory', href: '/admin/vehicles', icon: Car, badge: `${VEHICLES.length} Lots` },
    { label: 'Dealers CRM', href: '/admin/dealers', icon: Users, badge: `${DEALERS.length} Active` },
    { label: 'Calculation Engine', href: '/admin/settings', icon: Settings, badge: null },
    { label: 'Operations Guide', href: '/admin/help', icon: HelpCircle, badge: null },
  ];
  ...
  return (
    <div className="flex h-screen bg-[#F6F8FB] text-slate-800 font-sans antialiased overflow-hidden">
      ...
      {/* Sidebar - Matching Dealer Layout */}
      <aside className={`
      ...
      `}>
        <div>
          ...
          <div className="h-[70px] flex items-center justify-between px-4 sm:px-5 border-b border-[#1B2A4A]/60 bg-[#060E22]">
            <Link href="/admin" className="flex items-center gap-2.5 group" title="AutoHub Dealer Intelligence Platform (DIP) Admin">
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[8.5px] px-1.5 py-0.2 rounded font-extrabold bg-[#B30D12]/25 text-red-300 border border-[#B30D12]/40">ADMIN</span>
                </div>
          ...
          {/* Live JPY/NZD Rate Chip & Auction Scraper Feeds */}
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="text-[10px] text-slate-400 truncate flex items-center gap-1 mt-0.5">
                <Database size={10} className="shrink-0 text-emerald-400" />
                <span className="truncate">AutoHub Super Admin</span>
              </div>
            </div>
          </div>
          <Link
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative hidden md:flex items-center">
              ...
              <input
                ...
                placeholder="Search models, dealers, VINs, lot #..."
                className="pl-12 pr-12 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-[#B30D12] focus:ring-2 focus:ring-[#B30D12]/20 outline-none transition-all w-[380px] text-sm placeholder:text-slate-400 font-medium"
              />
            ...
            <div className="relative">
              <button
                ...
                title="Admin Feeds & Dispatch Alerts"
              >
                <Bell size={18} />
              ...
              {notificationsOpen && (
                <div className="absolute right-0 mt-2 w-84 bg-white rounded-2xl border border-slate-200 shadow-xl p-4 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                  <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-2">
                    <span className="text-xs font-bold text-slate-900">AutoHub Intelligence Alerts</span>
            ...
            <Link
              ...
              title="Admin Settings & Calculation Engine"
            >
        ...
        {!chatOpen && (
          <button
            ...
            title="Open AutoHub DIP Admin Copilot"
          >
            ...
            <div className="text-left">
              <div className="text-xs font-black tracking-wide flex items-center gap-1.5">
                AutoHub DIP Copilot
                <span className="text-[9px] px-1.5 py-0.2 rounded bg-[#B30D12]/30 text-red-200 border border-[#B30D12]/50 font-bold">Admin</span>
              </div>
              <div className="text-[10px] text-slate-300">
                Demand gap · Sourcing dispatch · Yields
              </div>
            </div>
          </button>
        )}
